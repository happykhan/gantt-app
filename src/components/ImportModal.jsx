    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--gx-border)', marginBottom: 20 }}>
          {['paste', 'file'].map(item => (
            <button key={item} onClick={() => { setTab(item); setPreview(null); setFileError(null) }}
        ...
        {tab === 'paste' && (
          <div>
            <textarea value={pasteText} onChange={event => { setPasteText(event.target.value); setPreview(null) }}
